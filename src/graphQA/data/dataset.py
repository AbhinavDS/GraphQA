...
import src.graphqa.utils.preprocess as preprocess_utils
import src.graphqa.utils.utils as utils
...
class GQADataset(Dataset):
	...
	def __getitem__(self, idx):
		...
		num_relations = len(relations_vocab_json)
		A = np.zeros((num_objects, num_objects * num_relations))
		...
		for obj_key in object_keys:
			...
			for rel_key in obj["relations"].keys():
				relation = obj["relations"]['rel_key']
				rel_encoded = preprocess_utils.encode(relation['name'],
												 self.vocab['relation_token_to_idx'],
												 allow_unk=True)
				obj_id = object_keys.getIndex(relation['object'])
				A_id = rel_encoded * num_objects + obj_id
		...
		return question_encoded, answer_endoded, A, objects, image_feat
