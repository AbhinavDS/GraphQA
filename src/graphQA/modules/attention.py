...
class TopDownAttention(nn.Module):
	...
	def __init__(self, args):
		...
		super(TopDownAttention, self, args, device).__init__()

		# Doubt on what to set
		self.attn_layer = nn.Linear(args.n_attn, 1)
		self.attn_gate = GatedTanh(args.n_img_feats + args.n_ques_emb, args.n_attn)
		self.attn_softmax = nn.Softmax(dim=1)
		self.device = device

	def forward(self, img_feats, ques_emb, num_obj):
		...
		# Computing attention
		gated_attn = self.attn_gate(torch.cat((img_feats, ques_emb.repeat(1, max_num_objs).reshape(batch_sz, max_num_objs, -1)), 2))
