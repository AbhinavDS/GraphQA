...
class GCN(nn.Module):
	def __init__(self, args, rel_word2vec=None, obj_name_word2vec=None):
		...
		if obj_name_word2vec is not None:
			...
			assert obj_name_word2vec.size(0) == self.max_num_objs
			assert obj_name_word2vec.size(1) == self.obj_emb_dim
			self.obj_name_embedding = nn.Embedding(self.max_num_objs, self.obj_name_emb_dim)
	...
	def forward(self, obj_feats, obj_rels):
		...
		batch_size, objects, _ = obj_feats.size(0)
		...
		print('Rel Emb Size: ', rel_embed.size())
		...
		for i in range(0,len(self.layers),2):
			...
			xr = self.layers[i+1](torch.mul(xr, rel_embed)).transpose(0, 2, 1, 3).view(batch_size, objects, -1)
